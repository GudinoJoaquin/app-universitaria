...
import {
  ...
  ActivityIndicator, StatusBar, KeyboardAvoidingView, Platform, StyleSheet, Dimensions, Switch
} from "react-native";
...
export default function EventCreateScreen({ route, navigation }) {
  ...
  const onStartTimeChange = (event, selectedTime) => {
    ...
    if (selectedTime) setFormData((p) => ({ ...p, start_time_raw: selectedTime }));
  };

  const onEndTimeChange = (event, selectedTime) => {
    setShowEndTimePicker(Platform.OS === "ios");
    if (selectedTime) setFormData((p) => ({ ...p, end_time_raw: selectedTime }));
  };
  ...
  return (
    <View style={s.container}>
      ...
      {showStartTimePicker && (
        <DateTimePicker value={formData.start_time_raw} mode="time" display={Platform.OS === "ios" ? "spinner" : "default"} is24Hour={true} onChange={onStartTimeChange} />
      )}
      {showEndTimePicker && (
        <DateTimePicker value={formData.end_time_raw} mode="time" display={Platform.OS === "ios" ? "spinner" : "default"} is24Hour={true} onChange={onEndTimeChange} />
      )}
    </View>
  );
}
...
const s = StyleSheet.create({
  ...
  header: { paddingTop: 60, paddingBottom: 20, borderBottomLeftRadius: 35, borderBottomRightRadius: 35 },
  ...
  headerTitle: { fontSize: 20, fontWeight: "800", color: "white", textAlign: "center" },
  ...
  imgIconBox: { width: 60, height: 60, borderRadius: 20, backgroundColor: "#EFF6FF", justifyContent: "center", alignItems: "center", marginBottom: 10 },
