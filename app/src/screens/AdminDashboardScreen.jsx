...
export default function AdminDashboardScreen({ navigation }) {
  ...
  const handleChangeRole = (targetUser) => {
    const allRoles = ["Admin", "Helper", "Organizer", "User"];
    Alert.alert(
      `Rol de ${targetUser.name}`,
      `Actual: ${targetUser.role}`,
      [
        ...allRoles.filter(r => r !== targetUser.role).map(r => ({
          text: r,
          onPress: async () => {
  ...
  const handleToggleState = async (state) => {
    ...
    const hasState = selectedUser.states?.some(s => s.id === state.id);
    const fn = hasState ? removeStateFromUser : assignStateToUser;
  ...
  return (
    <View style={s.container}>
      ...
      <LinearGradient colors={["#1E3A8A", "#3B82F6"]} style={s.header}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={s.headerTitle}>🛡️ Dashboard Admin</Text>
      ...
      <View style={s.quickActions}>
        {[
          ...
          { label: "Categorías", icon: "grid", color: "#EC4899", bg: "#FDF2F8", screen: "AdminCategories" },
            ...
            <View key={u.id} style={s.userCard}>
              ...
              <View style={s.actions}>
                ...
                <TouchableOpacity style={s.actionBtn} onPress={() => { setSelectedUser(u); setModalVisible(true); }}>
                  ...
                  <Text style={[s.actionTxt, { color: "#F59E0B" }]}>Gestionar estados</Text>
                </TouchableOpacity>
              </View>
            </View>
          );
