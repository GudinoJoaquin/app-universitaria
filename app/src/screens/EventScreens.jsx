...
const getEventStatus = (d) => {
  ...
  if (diff <= 1)
    return { label: "Hoy/Mañana", color: "#DC2626", bg: "#FEE2E2" };
  if (diff <= 7)
    return { label: "Esta semana", color: "#D97706", bg: "#FEF3C7" };
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const { data: eventsData } = await supabase
        .from("events")
        .select(
          "*, profiles(name, role, id), event_categories_junction(categories(name, color, id))",
        )
      ...
      const normalizedEvents = (eventsData ?? []).map((e) => ({
        ...
        categories: (e.event_categories_junction ?? [])
          .map((j) => j.categories)
          .filter(Boolean),
        profiles: undefined,
      ...
      const available = normalizedEvents.filter(
      ...
      if (isAdmin()) managed = normalizedEvents;
        ...
        managed = normalizedEvents.filter(
          (e) => !["Admin", "Helper"].includes(e.created_by_role),
        );
      else if (user?.role === "Organizer")
        managed = normalizedEvents.filter((e) => e.created_by === user.id);
      setMyManagedEvents(managed);

      const myRes = await getMyRegistrations(user.id);
      if (myRes.success) {
        setMyInscribedEvents(
          myRes.data.map((e) => ({
            ...e,
  ...
  const handleRegister = async (event) => {
    ...
    if (alreadyReg) {
      ...
      if (res.success) {
        ...
        setMyInscribedEvents((p) => [
          { ...event, registered_at: new Date().toISOString() },
          ...p,
        ]);
      } else Alert.alert("Error", res.error);
      setActionLoading((p) => ({ ...p, [event.id]: false }));
      ...
      >
        ...
        {isManaged ? (
          <View style={s.manageActions}>
            <TouchableOpacity
              style={[s.mBtn, { backgroundColor: "#EFF6FF" }]}
            ...
            <TouchableOpacity
              style={[s.mBtn, { backgroundColor: "#FFFBEB" }]}
              onPress={() =>
                navigation.navigate("CreateEvent", { event: item })
              }
            >
          ...
          status.label !== "Finalizado" && (
            <TouchableOpacity
              style={[s.regBtn, registered && s.regBtnOut]}
            ...
            >
              {opLoading ? (
                ...
                <>
                  ...
                  />
                  <Text
                    style={[s.regBtnTxt, registered && { color: "#EF4444" }]}
                  >
                    {registered ? "Cancelar" : "Inscribirse"}
                  </Text>
                </>
              )}
            </TouchableOpacity>
    ...
    <View style={s.container}>
      ...
      <View style={s.catContainer}>
        ...
        >
          ...
          >
            <Text
              style={[s.catChipTxt, !activeCategoryId && s.catChipTxtActive]}
            >
              Todos
            </Text>
          </TouchableOpacity>
          {categories.map((cat) => (
            <TouchableOpacity
              ...
              style={[
                s.catChip,
                activeCategoryId === cat.id && s.catChipActive,
              ]}
            ...
            >
              <Text
                style={[
                  s.catChipTxt,
                  activeCategoryId === cat.id && s.catChipTxtActive,
                ]}
              >
                {cat.name}
              </Text>
            ...
            {participantsLoading ? (
              ...
              <ScrollView>
                {participants.length === 0 ? (
                  <Text style={s.emptyP}>Aún no hay inscritos.</Text>
                ) : (
                  participants.map((p) => (
                    <View key={p.id} style={s.pRow}>
                      <View style={s.pAvatar}>
                        <Text style={s.pAvatarTxt}>{p.name?.[0] || "?"}</Text>
                      </View>
                      <View>
